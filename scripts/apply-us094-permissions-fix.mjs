...
const sql = fs
  .readFileSync(path.join(root, 'docs', 'US-094-permissions-fix.sql'), 'utf-8')
  .replace(/^\uFEFF/, '');
...
if (!supabaseUrl || !supabasePassword) {
  console.error('SUPABASE_URL/SUPABASE_PASSWORD ausentes');
  process.exit(1);
}
...
try {
  ...
  console.log('[sql] aplicando US-094-permissions-fix...');
