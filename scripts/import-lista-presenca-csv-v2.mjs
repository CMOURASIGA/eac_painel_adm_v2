...
const defaultInputDir = path.join(__dirname, '../documento projeto/carga inicial');
const inputPath = process.argv[2] || path.join(defaultInputDir, 'Lista de Presença (respostas) - Página3.csv');
const dryRun = !process.argv.includes('--apply');

const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY, {
  auth: { persistSession: false },
  global: { headers: { 'x-from': 'import-lista-presenca-csv-v2' } },
});
...
async function loadReferenceData() {
  ...
  const encontreirosView = await maybeFirstExistingTable(['vw_encontreiros', 'encontreiros'], 'pessoa_id,nome_completo,nome_normalizado,telefone,telefone_normalizado');
  const encontreiros = encontreirosView
    ? await fetchAll(encontreirosView, 'pessoa_id,nome_completo,nome_normalizado,telefone,telefone_normalizado', 50000)
    : [];
  ...
  const existingPresence = await fetchAll('presencas', 'id,data_presenca,telefone_normalizado,nome_digitado,payload', 100000);
  ...
  const registerPessoaCandidate = (candidate, idField = 'id') => {
    const id = String(candidate?.[idField] || '').trim();
    if (!id) return;
    const phoneKeys = [
      normalizePhone(candidate.telefone_normalizado),
      normalizePhone(candidate.telefone),
      digitsOnly(candidate.telefone_normalizado),
      digitsOnly(candidate.telefone),
    ].filter(Boolean);

    phoneKeys.forEach((key) => {
      if (!pessoaByPhone.has(key)) pessoaByPhone.set(key, { ...candidate, id });
    });

    const nameKey = normalizeName(candidate.nome_normalizado || candidate.nome_completo);
    if (!nameKey) return;
  ...
  for (const pessoa of pessoas) {
    registerPessoaCandidate(pessoa, 'id');
  }

  for (const encontreiro of encontreiros) {
    registerPessoaCandidate(encontreiro, 'pessoa_id');
  }
  ...
  existingPresence.forEach((row) => {
    ...
    const name = normalizeName(row.nome_digitado);
...
function findPessoaMatch(row, refs) {
  ...
  for (const key of phoneCandidates) {
    ...
    if (pessoa) return { pessoa, matchType: 'PHONE' };
  }

  const namedRaw = refs.pessoasByName.get(nameNorm) || [];
  const named = Array.from(new Map(namedRaw.map((item) => [String(item.id || ''), item])).values()).filter((item) => String(item.id || '').trim());
  if (named.length === 1) return { pessoa: named[0], matchType: 'NAME_EXACT' };

  if (named.length > 1 && phoneNorm) {
    const byPhone = named.find((pessoa) => normalizePhone(pessoa.telefone_normalizado || pessoa.telefone) === phoneNorm);
    if (byPhone) return { pessoa: byPhone, matchType: 'NAME_PHONE' };
  }
