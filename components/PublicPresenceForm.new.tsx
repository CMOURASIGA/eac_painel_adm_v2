...
const PublicPresenceForm: React.FC = () => {
  ...
  useEffect(() => {
    ...
    async function loadBase() {
      ...
      try {
        ...
        if (!r.success) throw new Error(r.error || 'Falha ao carregar lista de presenÃ§a.');
        if (!active) return;
        ...
        if (items.length === 0) {
          ...
          const msg = debug
            ? `Sem nomes para listar. Fontes: encontreiros=${debug.encontreirosCount}, encontristas=${debug.encontristasCount}, presenÃ§a=${debug.presenceCount}.`
        ...
        if (!active) return;
        const msg = e?.message || 'NÃ£o foi possÃ­vel carregar a base do formulÃ¡rio.';
        setError(msg);
        showToast(msg, 'error');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!selectedCandidate) {
      const msg = 'Selecione o nome para registrar a presenÃ§a.';
      setError(msg);
      showToast(msg, 'info');
      return;
    }
    ...
    if (forcarCorrecaoTelefone && !toCleanString(telefoneAtualizado)) {
      const msg = 'Informe o telefone atualizado ou desmarque a opÃ§Ã£o de correÃ§Ã£o.';
      setError(msg);
      showToast(msg, 'info');
      return;
    }
    ...
    if (forcarCorrecaoEmail && !toCleanString(emailAtualizado)) {
      const msg = 'Informe o e-mail atualizado ou desmarque a opÃ§Ã£o de correÃ§Ã£o.';
      setError(msg);
      showToast(msg, 'info');
      return;
    }
    ...
    if (semTelefone && !toCleanString(selectedCandidate.pessoaId) && !toCleanString(telefoneAtualizado)) {
      const msg = 'Este nome estÃ¡ sem identificador suficiente. Informe um telefone para registrar a presenÃ§a.';
      setError(msg);
      showToast(msg, 'info');
      return;
    }
    ...
    try {
      ...
      if (!r.success) throw new Error((r.raw as any)?.error || r.error || 'NÃ£o foi possÃ­vel registrar presenÃ§a.');
      ...
      showToast((r.data as any)?.message || 'PresenÃ§a registrada com sucesso!', 'success');
    } catch (e: any) {
      const msg = e?.message || 'Falha ao registrar presenÃ§a.';
      setError(msg);
      showToast(msg, 'error');
    } finally {
      setIsLoading(false);
      ...
      <div className="mx-auto w-full max-w-2xl">
        <div className="rounded-[28px] border border-slate-200 bg-white shadow-[0_12px_34px_-20px_rgba(15,23,42,0.45)] overflow-hidden">
          ...
          <div className="p-7 md:p-8">
            <h1 className="text-3xl font-black text-slate-900 text-center mb-2">Registro de PresenÃ§a</h1>
            <p className="text-center text-slate-600 mb-7">Selecione o evento e o nome para confirmar sua presenÃ§a.</p>

            {isSubmitted && (
              <div className="mb-5 rounded-xl border border-emerald-200 bg-emerald-50 p-4 text-emerald-800 font-semibold">
                PresenÃ§a registrada com sucesso.
              </div>
            )}
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                >
                  <option value="POS_ENCONTRO">PÃ³s-Encontro (encontreiro + encontrista)</option>
                  <option value="REUNIAO_CIRCULO">ReuniÃ£o de CÃ­rculo (somente encontrista)</option>
                </select>
              </div>
              ...
              <div>
                <label className="block text-sm font-extrabold text-slate-800 mb-1">CÃ­rculo *</label>
                <input
                  value={circulo}
                  onChange={(e) => setCirculo(e.target.value)}
                  disabled={isLockedEncontreiroCircle}
                  placeholder="Ex.: Azul / CÃ­rculo 1"
                ...
                {isLockedEncontreiroCircle ? (
                  <p className="mt-1 text-xs font-semibold text-slate-500">
                    CÃ­rculo definido automaticamente como Encontreiro para este tipo de evento.
                  </p>
                ) : null}
              </div>
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <label className="flex items-start gap-3 text-sm font-semibold text-slate-700">
                  ...
                  />
                  <span>Meu telefone estÃƒÂ¡ ausente ou desatualizado neste cadastro.</span>
                </label>
                {!toCleanString(selectedCandidate?.telefone) ? (
                  <p className="mt-2 text-xs font-semibold text-amber-700">
                    Este nome estÃƒÂ¡ sem telefone vinculado. Se quiser, informe agora para atualizar o cadastro junto com a presenÃ§a.
                  </p>
                ) : null}
              ...
              <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <label className="flex items-start gap-3 text-sm font-semibold text-slate-700">
                  <input
                    ...
                    className="mt-1 h-4 w-4 rounded border-slate-300 text-blue-600"
                  />
                  <span>Meu e-mail estÃƒÂ¡ ausente ou desatualizado neste cadastro.</span>
                </label>
                {!toCleanString(selectedCandidate?.email) ? (
                  <p className="mt-2 text-xs font-semibold text-amber-700">
                    Este nome estÃƒÂ¡ sem e-mail vinculado. Se quiser, informe agora para atualizar o cadastro junto com a presenÃ§a.
                  </p>
                ) : null}
              ...
              >
                {isLoading ? 'Registrando presenÃ§a...' : 'Registrar presenÃ§a'}
              </button>
